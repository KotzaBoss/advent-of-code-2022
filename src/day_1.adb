...
procedure Day_1 is
   ...
   -- Insertion sort
   procedure Store_If_In_Top_3 (Self: in out Top_3_Calories_Record; C : in Calories) is
   begin
      for I in Self.Top_3'Range loop
         if C > Self.Top_3 (I) then declare
               -- Delta is what what comes - what leaves
               Calories_Delta : constant Calories := C - Self.Top_3 (Self.Top_3'Last);
            begin
               Self.Sum := @ + Calories_Delta;
               -- Shift 1 to the right
               for J in reverse I + 1 .. Self.Top_3'Last loop
                  Self.Top_3 (J) := Self.Top_3 (J - 1);
               end loop;
               Self.Top_3 (I) := C;
               exit;
            end;
   ...
   package Calories_Vectors is new
     Ada.Containers.Vectors
       (Index_Type => Natural,
        Element_Type => Calories);
   use Calories_Vectors;

   function Sum_Calories (V : in Calories_Vectors.Vector) return Calories is
      Sum : Calories := 0;
   begin
      for C of V loop
         Sum := @ + C;
   ...
   begin
      if Line'Size /= 0 then
         Calories_Vector.Append (Calories'Value (Line));
            ...
            Sum : constant Calories := Sum_Calories (Calories_Vector);
         begin
            Put_Line ("Calories: " & Calories_Vector'Image & LF
                      & "Sum:" & Sum'Image);
            Store_If_In_Top_3 (Top_3_Calories, Sum);
            Put_Line ("Top 3: " & Top_3_Calories'Image);
            New_Line;
   ...
   procedure Parse_Calories is new AWK.For_Every_Line_Current_Session (Action);

begin
   Parse_Calories(Separators => AWK.Use_Current,
