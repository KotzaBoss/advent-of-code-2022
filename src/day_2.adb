...
procedure Day_2 is
   ...
   package Choices is
      ...
      function Cheat (Opponent : in Pure; Outcome : in Choices.Outcome) return Pure;
   end Choices;
   ...
   package body Choices is
      function From_String (S : in String) return Outcome is
         ...
         Outcome       : constant Choices.Outcome   := Choices.Outcome'Val (Encrypted_Pos);
      begin
         return Outcome;
      end From_String;
      ...
      function Cheat (Opponent : in Pure; Outcome : in Choices.Outcome) return Pure is
         Outcome_Choice_Offsets : constant array (Loss .. Win) of Integer := (-1, 0, 1);
         ------------------
         Opponent_Pos : constant Natural := Pure'Pos (Opponent);
         Choice_Pos   : constant Natural := (Opponent_Pos + Outcome_Choice_Offsets(Outcome)) mod 3;
      begin
   ...
   procedure Action is
      type Round is record
         ...
         Outcome_Value       : Natural;
         Opponent, Player    : Choices.Pure;
         Choice_Value, Score : Natural;
      end record;
      -- Just for the convenient 'Image
      ...
      R : constant Round := (Outcome, Outcome_Value, Opponent, Player, Choice_Value, Score);
   begin
      Total_Score := @ + Score;
      Put_Line (R'Image & Total_Score'Image);
   end Action;

begin
   AWK.Register(Field => 0,
                Pattern => Regpat.Compile("[A-C] [X-Z]"),
